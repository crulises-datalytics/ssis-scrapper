...
CREATE PROCEDURE [dbo].[spMISC_SourceToStaging_WeeklyPlanAllocationRevenue]
(
   @AuditId		   bigint = NULL
...
AS
   -- ================================================================================
   -- 
   -- Stored Procedure:   spMISC_SourceToStaging_WeeklyPlanAllocationRevenue
   --
   -- Purpose:            Performs the Insert / Update / Delete ETL process for
   --                         the WeeklyPlanAllocationRevenueLanding table from Source 
   ...
   --                         core logic.			   
   --
   -- Usage:              EXEC dbo.spCMS_SourceToStaging_ARBalanceSnapshot @DebugMode = 1	
   -- 
   --
   -- --------------------------------------------------------------------------------
   --
   ...
   --                                     WeeklyPlanAllocationRevenue table in 
   --                                     MISC_Staging
   --  8/26/2022   Banandesi		 BI-6476 Weekly Allocation Update
   -- 09/02/2022   Adevabhakthuni    Added the reverted code 
   -- ================================================================================
     BEGIN
         SET NOCOUNT ON;
         ...
         DECLARE @SourceName varchar(100) = 'WeeklyPlanAllocationRevenue';
     ...
     BEGIN TRY
         ...
         -- Insert data     
         -- 
         IF OBJECT_ID('tempdb..#WeeklyPlanAllocationRevCurrent') IS NOT NULL
                 DROP TABLE #WeeklyPlanAllocationRevCurrent;
              SELECT  ISNULL(LEFT(misc_pa.CostCenter,6),-1) AS CostCenterNumber
                    ...
                    ISNULL(CONVERT(decimal(19, 8), CAST(misc_pa.Allocation AS float)), 0) AS AllocationPercentage,
                    'Dollars' AS GLMetricTypeName,
                    SUM(ISNULL(CONVERT(decimal(19, 8), CAST(misc_pa.Allocation AS float)), 0)) OVER(PARTITION BY dd.FiscalPeriodNumber,
                                                                                                                 LEFT(misc_pa.CostCenter, 6),
                                                                                                                 LEFT(misc_pa.AccountSubAccount, 11)) AS PeriodTotal
             INTO #WeeklyPlanAllocationRevCurrent
             FROM dbo.WeeklyPlanAllocationRevenueLanding AS misc_pa
             ...
             WITH CTEPeriodicPlanAllocation
                  ...
                  SELECT DISTINCT
                         ...
                         GLMetricTypeName,
                         FiscalPeriodNumber
                  FROM #WeeklyPlanAllocationRevCurrent)
                  DELETE t
                  FROM dbo.WeeklyPlanAllocationCurrent AS t
                       ...
                       LEFT JOIN #WeeklyPlanAllocationRevCurrent AS s ON s.CostCenterNumber = t.CostCenterNumber
		 ...
		 -- Merge all other data							
              ...
              USING #WeeklyPlanAllocationRevCurrent AS s
