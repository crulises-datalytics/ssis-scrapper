...
(
    ...
    FROM sys.objects
    WHERE type = 'P'
          AND name = 'spCMS_StagingToEDW_FactSessionEnrollment'
)
    DROP PROCEDURE dbo.spCMS_StagingToEDW_FactSessionEnrollment;
GO
*/
CREATE PROCEDURE dbo.spCMS_StagingToEDW_FactSessionEnrollment
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_StagingToEDW_FactSessionEnrollment
    --
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @TaskName VARCHAR(100)= 'FactSessionEnrollment';
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
		   -- ================================================================================
             CREATE TABLE #FactSessionEnrollmentUpsert
             ([EnrollmentID]                      [INT] NOT NULL,
              [ProgramEnrollmentID]               [INT] NOT NULL,
              [SessionEnrollmentID]               [INT] NOT NULL,
              [OrgKey]                            [INT] NOT NULL,
              ...
              [SessionEnrollmentEffectiveDateKey] [INT] NOT NULL,
              [SessionEnrollmentEndDateKey]       [INT] NOT NULL,
              [DailyFTE]                          [NUMERIC](5, 3) NOT NULL,
		   ...
		   --
		   -- ================================================================================
             INSERT INTO #FactSessionEnrollmentUpsert
		   ...
		   -- Get how many rows were extracted from source 
             ...
             FROM #FactSessionEnrollmentUpsert;
		   ...
		   -- Create helper index
		   --
             CREATE NONCLUSTERED INDEX XAK1FactSessionEnrollmentUpsert ON #FactSessionEnrollmentUpsert
             ([EnrollmentID] ASC, [ProgramEnrollmentID] ASC, [SessionEnrollmentID] ASC
             );
		   ...
		   --
             MERGE [BING_EDW].[dbo].[FactSessionEnrollment] T
             USING #FactSessionEnrollmentUpsert S
             ON(S.EnrollmentID = T.EnrollmentID
                AND S.ProgramEnrollmentID = T.ProgramEnrollmentID
                AND S.SessionEnrollmentID = T.SessionEnrollmentID
                AND S.SessionEnrollmentEffectiveDateKey = T.SessionEnrollmentEffectiveDateKey)
                 WHEN MATCHED AND S.Deleted IS NULL
                                  AND (S.OrgKey <> T.OrgKey
                                       ...
                                       OR S.ScheduleWeekKey <> T.ScheduleWeekKey
                                       OR S.ClassroomKey <> T.ClassroomKey
                                       OR S.SessionEnrollmentEffectiveDateKey <> T.SessionEnrollmentEffectiveDateKey
                                       OR S.SessionEnrollmentEndDateKey <> T.SessionEnrollmentEndDateKey
                                       OR S.DailyFTE <> T.DailyFTE
                 ...
                 THEN UPDATE SET
                                 ...
                                 T.ScheduleWeekKey = S.ScheduleWeekKey,
                                 T.ClassroomKey = S.ClassroomKey,
                                 T.SessionEnrollmentEffectiveDateKey = S.SessionEnrollmentEffectiveDateKey,
                                 T.SessionEnrollmentEndDateKey = S.SessionEnrollmentEndDateKey,
                                 T.DailyFTE = S.DailyFTE,
                                 T.EDWCreatedDate = S.EDWCreatedDate,
                                 T.EDWModifiedDate = @EDWRunDateTime,
                                 T.Deleted = S.Deleted
                 ...
                 THEN
                   INSERT(EnrollmentID,
                          ...
                          ScheduleWeekKey,
                          ClassroomKey,
                          SessionEnrollmentEffectiveDateKey,
                          SessionEnrollmentEndDateKey,
                          DailyFTE,
                          EDWCreatedDate,
                          EDWModifiedDate,
                          Deleted)
		  ...
		  -- Debug output progress
		   ...
		   -- Drop the temp table
		   --
             DROP TABLE #FactSessionEnrollmentUpsert;
