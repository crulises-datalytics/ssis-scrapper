CREATE PROCEDURE dbo.spCSS_StagingToEDW_DimDiscountType
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCSS_StagingToEDW_DimDiscountType
    --
    ...
    --
    -- Usage:              EXEC dbo.spCSS_StagingToEDW_DimDiscountType @DebugMode = 1	
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimDiscountType - CSS';
	    ...
	    --
         ...
         -- Create helper index
         --
         CREATE NONCLUSTERED INDEX XAK1DimDiscountTypeUpsert ON #DimDiscountTypeUpsert
         ([SourceSystem] ASC, [CSSTransactionType] ASC, [CSSTransactionCode] ASC
         );
		   ...
		   --
             ...
             ON(S.SourceSystem = T.SourceSystem
                ...
                AND S.CSSTransactionCode = T.CSSTransactionCode)
                 WHEN MATCHED AND(S.DiscountTypeID <> T.DiscountTypeID
                                  OR S.DiscountTypeName <> T.DiscountTypeName
                 ...
                 THEN UPDATE SET
                                 T.DiscountTypeID = S.DiscountTypeID,
