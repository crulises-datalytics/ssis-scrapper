CREATE PROCEDURE [dbo].[spHR_StagingToEDW_FactEmployeeLeave]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spHR_StagingToEDW_FactEmployeeLeave
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --					  the FactEmployeeLeave table from Staging to BING_EDW.
    --
    ...
    --
    --
    -- Usage:              EXEC dbo.spHR_StagingToEDW_FactEmployeeLeave @EDWRunDateTime = @EDWRunDateTime, @DebugMode = 1
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
    ...
    -- ----		-----------		--------
    --
    -- 3/19/18  hhebbalu		BNG-279 - Intital version of the proc
    --			 
    -- ================================================================================
     BEGIN
         SET NOCOUNT ON;
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'FactEmployeeLeave';
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
             ...
             FROM BING_EDW.dbo.FactEmployeeLeave;
             
			 Truncate table BING_EDW.dbo.FactEmployeeLeave
		   ...
		   --
             INSERT INTO BING_EDW.dbo.FactEmployeeLeave
             ( 
				 [LeaveActualStartDateKey]
				,[LeaveActualEndDateKey]
				,[LeaveProjectedStartDateKey]
				,[LeaveProjectedEndDateKey]
				,[LeaveTypeKey]
				,[LeaveReasonKey]
				,[PersonKey]
				,[EmployeeLeaveID]
              ...
              EXEC [dbo].[spHR_StagingTransform_FactEmployeeLeave];
