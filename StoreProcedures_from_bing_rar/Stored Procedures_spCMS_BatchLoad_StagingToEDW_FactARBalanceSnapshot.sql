CREATE PROCEDURE [dbo].[spCMS_BatchLoad_StagingToEDW_FactARBalanceSnapshot]
	@EDWRunDateTime DATETIME2 = NULL,
	@DebugMode      INT       = NULL
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_BatchLoad_StagingToEDW_FactARBalanceSnapshot
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --					  the FactARBalanceSnapshot table from Staging to BING_EDW, loading
    ...
    --                         core logic.
    --                     @DaysBack - For FactARBalanceSnapshot load we have to go back and potentially reporcess
    ...
    --
    -- Usage:              EXEC dbo.spCMS_BatchLoad_StagingToEDW_FactARBalanceSnapshot @EDWRunDateTime = @EDWRunDateTime, @DebugMode = 1
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
    ...
    -- ----				-----------         --------
    --
    -- 10/30/17			sburke              BNG-247. Load of FactARBalanceSnapshot into EDW.
    -- 07/26/19			hhebbalu            BI-1653/BI-1654/BI-1655 Removed the 14 days delete logic
	--										and modified code to load data from the min week changed in Staging till today.
    ...
    -- ================================================================================
     BEGIN
		...
		*/
	    ...
	    --
		DECLARE @StartingFiscalWeekEndDate DATETIME;
		...
		WHERE EventName = @TaskName
         ...
         BEGIN TRY
		   ...
		   -- ================================================================================
				...
				SET @StartingFiscalWeekEndDate =
				(
					SELECT MIN(CAST(AsOfFiscalWeekEndDate AS DATE))
					FROM dbo.ARBalanceSnapshot
					WHERE StgCreatedDate > @LastProcessedDate
				); -- Use @StartingFiscalWeekEndDate as the earliest dated transaction in CMS. No point going further back than this
		   ...
		   -- ================================================================================
             DECLARE csr_fact_rabalance_fisc_weekend CURSOR
             FOR
		   ...
		   --
             ...
             FETCH NEXT FROM csr_fact_rabalance_fisc_weekend INTO @FiscalWeekEndDate;
             WHILE @@FETCH_STATUS = 0
				 ...
				 -- ================================================================================
                     EXEC dbo.spCMS_StagingToEDW_FactARBalanceSnapshot
                          @AuditId = @AuditId,
