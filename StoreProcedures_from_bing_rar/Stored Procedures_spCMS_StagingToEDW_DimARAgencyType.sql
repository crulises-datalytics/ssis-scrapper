...
CREATE PROCEDURE [dbo].[spCMS_StagingToEDW_DimARAgencyType]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_StagingToEDW_DimARAgencyType
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --                         the DimARAgencyType table from Staging to BING_EDW.
    --
    --                         Step 1: Create temporary landing #table
    --                         Step 2: Populate the Landing table from Source by calling
    --                                 sub-procedure spHR_StagingTransform_DimARAgencyType, 
    ...
    --
    -- Usage:              EXEC dbo.spCMS_StagingToEDW_DimARAgencyType @DebugMode = 1	
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimARAgencyType - CMS';
	    ...
	    --
         ...
         --
         -- ================================================================================        
	    CREATE TABLE #DimARAgencyTypeUpsert (
		[ARAgencyTypeName] VARCHAR(100) NOT NULL,
		[ARType]           VARCHAR(100) NOT NULL,
		...
		[EDWCreatedBy]     VARCHAR(50) NOT NULL
         ...
         PRINT @DebugMsg;
		   ...
		   -- Create helper index
		   --
         CREATE NONCLUSTERED INDEX XAK1DimARAgencyTypeUpsert ON #DimARAgencyTypeUpsert
         ([ARAgencyTypeName] ASC, [SourceSystem] ASC
         );
	    -- --------------------------------------------------------------------------------
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
             MERGE [BING_EDW].[dbo].[DimARAgencyType] T
             USING #DimARAgencyTypeUpsert S
             ON(S.SourceSystem = T.SourceSystem
                AND S.ARAgencyTypeName = T.ARAgencyTypeName)
                 WHEN MATCHED AND(S.ARType <> T.ARType)
                 THEN UPDATE SET
                                 T.ARType = S.ARType
                 WHEN NOT MATCHED BY TARGET
                 THEN
                   INSERT(ARAgencyTypeName,
                          ARType,
                          SourceSystem,
                          EDWCreatedDate,
                          EDWCreatedBy)
