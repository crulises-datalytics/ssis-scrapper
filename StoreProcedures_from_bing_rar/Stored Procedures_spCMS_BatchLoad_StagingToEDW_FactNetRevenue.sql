CREATE PROCEDURE [dbo].[spCMS_BatchLoad_StagingToEDW_FactNetRevenue]
	@EDWRunDateTime DATETIME2 = NULL, 
	@DebugMode      INT       = NULL
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_BatchLoad_StagingToEDW_FactNetRevenue
    --
    ...
    --
    --
    --Usage:              EXEC dbo.spCMS_BatchLoad_StagingToEDW_FactNetRevenue
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
    ...
    -- 11/12/18			sburke              BNG-4424 - Add logic to look for records updated beyond the previous 2 weeks
    --					                        and include in the batch load
    ...
    -- 01/30/20			Adevabhakthuni		BI-3008 Updated the Stored Proc to delete and load previous week data  
    ...
    BEGIN
		...
		*/
        ...
        BEGIN TRY
            ...
            -- 2.) Looking at the finGeneralLedgerLanding
            -- ================================================================================
            ...
            DECLARE @SourceFiscalDate TABLE(TransactionDate DATE);
            INSERT INTO @SourceFiscalDate
				SELECT DISTINCT 
            ...
            FROM BING_EDW.dbo.DimDate(NOLOCK) d
                JOIN @SourceFiscalDate s ON d.FullDate = s.TransactionDate
            WHERE FiscalWeekStartDate <= @EDWRunDateTime
                    AND FiscalPeriodOfYearName <> 'ADJ'
            UNION

            /*Added to get last weekenddate as BI-3008*/

            SELECT DISTINCT 
                    FiscalWeekEndDate
            FROM BING_EDW..DimDate
            WHERE FiscalWeekEndDate BETWEEN @LastProcessedDate AND CONVERT(DATE, GETDATE());
            ...
            WHILE @@FETCH_STATUS = 0
                BEGIN
                    ...
                    -- ================================================================================
                    EXEC dbo.spCMS_StagingToEDW_FactNetRevenue 
                         @AuditId = @AuditId, 
