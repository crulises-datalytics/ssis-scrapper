CREATE PROCEDURE dbo.spCMS_StagingToEDW_DimCreditMemoType
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_StagingToEDW_DimCreditMemoType
    --
    ...
    --
    -- Usage:              EXEC dbo.spCMS_StagingToEDW_DimCreditMemoType @DebugMode = 1	
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimCreditMemoType';
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
		   -- ================================================================================
             CREATE TABLE #DimCreditMemoTypeUpsert
             ([CreditMemoTypeID]      INT NOT NULL,
              [CreditMemoTypeName]    VARCHAR(100) NOT NULL,
              [CreditMemoCategory]    VARCHAR(25) NOT NULL,
              [CreditMemoPostingType] VARCHAR(10) NOT NULL,
		   ...
		   --
		   -- ================================================================================
             INSERT INTO #DimCreditMemoTypeUpsert
             EXEC dbo.spCMS_StagingTransform_DimCreditMemoType
                  @EDWRunDateTime;
		   ...
		   -- Create helper index
		   --
             CREATE NONCLUSTERED INDEX XAK1DimCreditMemoTypeUpsert ON #DimCreditMemoTypeUpsert
             ([CreditMemoTypeID] ASC
             );
		   ...
		   --
             MERGE [BING_EDW].[dbo].[DimCreditMemoType] T
             USING #DimCreditMemoTypeUpsert S
             ON(S.CreditMemoTypeID = T.CreditMemoTypeID)
                 WHEN MATCHED AND(S.CreditMemoTypeName <> T.CreditMemoTypeName
                                  OR S.CreditMemoCategory <> T.CreditMemoCategory
                                  OR S.CreditMemoPostingType <> T.CreditMemoPostingType)
                                 AND S.Deleted IS NULL
                 THEN UPDATE SET
                                 T.CreditMemoTypeName = S.CreditMemoTypeName,
                                 T.CreditMemoCategory = S.CreditMemoCategory,
                                 T.CreditMemoPostingType = S.CreditMemoPostingType,
		  ...
		  -- Debug output progress
		   ...
		   -- Drop the temp table
		   --
             DROP TABLE #DimCreditMemoTypeUpsert;
