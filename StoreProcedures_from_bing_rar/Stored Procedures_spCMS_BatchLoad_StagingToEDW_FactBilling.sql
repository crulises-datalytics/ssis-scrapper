CREATE PROCEDURE dbo.spCMS_BatchLoad_StagingToEDW_FactBilling
	@EDWRunDateTime DATETIME2 = NULL,
	@DebugMode      INT       = NULL
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_BatchLoad_StagingToEDW_FactBilling
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --					  the FactBilling table from Staging to BING_EDW, loading
    ...
    --
    --
    -- Usage:              EXEC dbo.spCMS_BatchLoad_StagingToEDW_FactBilling @EDWRunDateTime = @EDWRunDateTime, @DebugMode = 1
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
		...
		*/
		...
		BEGIN TRY
		   ...
		   --
             ...
             SELECT DISTINCT
				YEAR(InvoiceDate)
             FROM [CMS_Staging].[dbo].[finStudentInvoice];
		   ...
		   -- ================================================================================
             DECLARE csr_fact_billing_fiscal_year CURSOR
             FOR
		   ...
		   --
             OPEN csr_fact_billing_fiscal_year;
             FETCH NEXT FROM csr_fact_billing_fiscal_year INTO @FiscalYearNumber;
             WHILE @@FETCH_STATUS = 0
				 ...
				 -- ================================================================================
                     EXEC dbo.spCMS_StagingToEDW_FactBilling
                          @AuditId = @AuditId,
                          @FiscalYearNumber = @FiscalYearNumber;
                     FETCH NEXT FROM csr_fact_billing_fiscal_year INTO @FiscalYearNumber;
                 END;
             CLOSE csr_fact_billing_fiscal_year;
             DEALLOCATE csr_fact_billing_fiscal_year;
