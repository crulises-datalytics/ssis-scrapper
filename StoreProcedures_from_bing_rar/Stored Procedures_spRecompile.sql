...
(
    ...
    FROM sys.objects
    WHERE type = 'P'
          AND name = 'spRecompile'
)
    DROP PROCEDURE dbo.spRecompile;
GO
--*/

CREATE PROCEDURE [dbo].[spRecompile]
...
AS
    ...
    --                         core logic.
    --
    --Usage:              EXEC dbo.spRecompile @DebugMode = 1
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
    ...
    -- ================================================================================
     BEGIN
         ...
         DECLARE @TaskName VARCHAR(100)= 'spRecompile - '+DB_NAME();
         ...
         BEGIN TRY
             DECLARE @SpRecompileSQLCommand VARCHAR(500);
             DECLARE @SpRecompileTable TABLE(UpdateStatsSQL VARCHAR(500) NOT NULL);
		   -- Build a list of SQL commands to Update Stats for each table in the database
             INSERT INTO @SpRecompileTable
		   ...
		   --
             ...
             FOR
                 ...
                 FROM @SpRecompileTable
                 ORDER BY UpdateStatsSQL;             
		   --
		   -- Use cursor to loop through the values we have chosen to split this batch by
		   --
             OPEN csr_update_stats;
             FETCH NEXT FROM csr_update_stats INTO @SpRecompileSQLCommand;
             WHILE @@FETCH_STATUS = 0
                 BEGIN
                     SELECT @DebugMsg = @ProcName+' : '+CONVERT(NVARCHAR(20), GETDATE())+' - Executing SQL command... '+@SpRecompileSQLCommand;
                     EXEC (@SpRecompileSQLCommand);
