CREATE PROCEDURE [dbo].[spGL_StagingTransform_DimAccountSubAccount] @EDWRunDateTime DATETIME2 = NULL
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spGL_StagingTransform_DimAccountSubAccount
    --
    ...
    --                     EXEC dbo.spGL_StagingTransform_DimAccountSubAccount @EDWRunDateTime
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    --                              accounts in SSAS. 
	--
    -- 03/06/20    tyj				BI-3443 Got rid of the TFN used as the source table call and moved
    --                              the new logic directly into this stored procedure so that we could 
    --                              use an interrim temp table (#AccSubHierarchy_DimAccountSubaccount) 
    --                              in building the result set to fix performance instead of using only CTEs. 
    --                              Ultimately we replaced the TFN result as a temp table as well 
    --                              (#AccSubHierarchy_DimAccountSubaccount)
	--			   
    -- 04/02/21    hhebbalu         BI-4501 Added the FieldPath logic in the Select query at the end                        
	-- 06/24/22    Suhas.DeMail-AM	BI-2161 Added RowStatus as 'A' in line 1440 (final select statement)
    -- ================================================================================
     BEGIN
         SET NOCOUNT ON;
                   --
...
AccountSubaccountIDs    AS (
    SELECT DISTINCT
        AccountSubaccountID = Segment2+'.'+Segment4,
        AccountID = Segment2,
        SubaccountID = Segment4,
        -- AccountTypeCode, AccountTypeName & AccountTypeUnary to determine signage of Revenue Accounts
        AccountTypeCode = ACCOUNT_TYPE,
        AccountTypeName = 
                                             CASE ACCOUNT_TYPE
                                                            WHEN 'R' THEN 'Revenue'
                                                            WHEN 'O' THEN 'Owner''s Equity'
        ...
        AccountTypeUnary = 
                                             CASE
                WHEN ACCOUNT_TYPE IN('E', 'A')
                THEN-1
                ELSE 1
            END
    FROM GLCodeCombinations
    UNION
    SELECT DISTINCT
            AccountSubaccountID,
            LEFT(AccountSubaccountID, 4),
            RIGHT(AccountSubaccountID, 6),
            NULL, -- AccountTypeCode
            NULL, -- AccountTypeName
            NULL -- AccountTypeUnary
    FROM AccountSubaccountHierarchyAttributes
    WHERE RowType = 'Stub'),
