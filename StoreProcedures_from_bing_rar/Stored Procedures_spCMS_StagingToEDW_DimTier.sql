CREATE PROCEDURE [dbo].[spCMS_StagingToEDW_DimTier]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_StagingToEDW_DimTier
    --
    ...
    --                                 sub-procedure spCMS_StagingTransform_DimTier, 
    ...
    --
    -- Usage:              EXEC dbo.spCMS_StagingToEDW_DimTier @DebugMode = 1
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
    ...
    -- ----        -----------         --------
    --
    -- 2/19/18     Adevabhakthuni             BNG-1242 - Refactor DimTier ETL so it uses 
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimTier';
	...
	[Deleted] [datetime2](7) NULL
		   ...
		   -- Create helper index
		   --
             CREATE NONCLUSTERED INDEX XAK1DimFeeType ON #DimTier
             ([TierID] ASC
             );
		   ...
		   --
             ...
             USING #DimTier S
             ON(S.TierID = T.TierID)
                 WHEN MATCHED AND S.Deleted IS NULL
                                  AND T.SourceSystem = 'CMS'
                                  AND ( 
								   T.TierName <> S.TierName
								OR T.TierFriendlyName <> S.TierFriendlyName
								OR T.TierAssignment <> S.TierAssignment
								OR T.TierBillingFrequency <> S.TierBillingFrequency
								OR T.TierLabel <> S.TierLabel
								OR T.TierShowToSponsor <> S.TierShowToSponsor
								OR T.CSSTierNumber <> S.CSSTierNumber
                 ...
                 THEN UPDATE SET
								...
								T.CSSTierNumber = S.CSSTierNumber ,
