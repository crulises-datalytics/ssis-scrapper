CREATE PROCEDURE [dbo].[spHR_StagingToEDW_FactEmployeePayRate]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spHR_StagingToEDW_FactEmployeePayRate
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --					  the FactEmployeePayRate table from Staging to BING_EDW.
    --
    ...
    --
    --
    -- Usage:              EXEC dbo.spHR_StagingToEDW_FactEmployeePayRate @EDWRunDateTime = @EDWRunDateTime, @DebugMode = 1
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
    ...
    -- ----		-----------		--------
    --
    -- 3/2/18  valimineti		    BNG-274 - Intial version of EDW FactEmployeePayRate load
    --			 
    -- ================================================================================
     BEGIN
         SET NOCOUNT ON;
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'FactEmployeePayRate';
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
             ...
             FROM BING_EDW.dbo.FactEmployeePayRate;
             
			 Truncate table BING_EDW.dbo.FactEmployeePayRate
		   ...
		   --
             INSERT INTO BING_EDW.dbo.FactEmployeePayRate
             ( [EmployeePayRateEffectiveDateKey]
			  ,[EmployeePayRateEndDateKey]
			  ,[EmployeePayRateCurrentRecordFlag]
			  ,[PersonKey]
			  ,[LocationKey]
			  ,[OrgKey]
			  ,[CompanyKey]
			  ...
			  ,[AssignmentID]
			  ,[PayRateApprovedFlag]
			  ,[EmployeeAnnualizedPayRate]
			  ,[EmployeeHourlyPayRate]
			  ,[EmployeeSalaryPayRate]
			  ,[EmployeePayRateCreatedDate]
			  ,[EmployeePayRateCreatedUser]
			  ,[EmployeePayRateModifiedDate]
			  ,[EmployeePayRateModifiedUser]
			  ,[EDWCreatedDate]
			  ,[EDWModifiedDate]
             )
              EXEC [dbo].[spHR_StagingTransform_FactEmployeePayRate];
