...
(
    ...
    FROM sys.objects
    WHERE type = 'P'
          AND name = 'spCMS_StagingToEDW_DimDiscountType'
)
    DROP PROCEDURE dbo.spCMS_StagingToEDW_DimDiscountType;
GO
*/
CREATE PROCEDURE [dbo].[spCMS_StagingToEDW_DimDiscountType]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_StagingToEDW_DimDiscountType
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --                         the DimDiscountType table from Staging to BING_EDW.
    --
    ...
    --
    -- Usage:              EXEC dbo.spCMS_StagingToEDW_DimDiscountType @DebugMode = 1	
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    -- ----        -----------         --------
    --
    -- 12/11/17    anmorales           BNG-1235 - Refactor DimDiscountType ETL so it 
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimDiscountType';
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
		   -- ================================================================================
           CREATE TABLE #DimDiscountTypeUpsert (
             [DiscountTypeID]          INT               NOT NULL,
             [DiscountTypeName]        VARCHAR(100)      NOT NULL,
             [DiscountTypeDescription] VARCHAR(500)      NOT NULL,
             [DiscountCategory]        VARCHAR(100)      NOT NULL,
             [DiscountRecurring]       VARCHAR(50)       NOT NULL,
             [DiscountPriority]        INT               NOT NULL,
             [DiscountNet]             VARCHAR(25)       NOT NULL,
		   ...
		   --
		   -- ================================================================================
             INSERT INTO #DimDiscountTypeUpsert
		   ...
		   -- Create helper index
		   --
             CREATE UNIQUE NONCLUSTERED INDEX XAK1DimDiscountTypeUpsert ON #DimDiscountTypeUpsert
             ([DiscountTypeID] ASC
             );
		   ...
		   --
             MERGE [BING_EDW].[dbo].[DimDiscountType] T
             USING #DimDiscountTypeUpsert S
             ON(S.DiscountTypeID = T.DiscountTypeID)
                 WHEN MATCHED AND(S.DiscountTypeName <> T.DiscountTypeName
                                 OR S.DiscountTypeDescription <> T.DiscountTypeDescription
                                 OR S.DiscountCategory <> T.DiscountCategory
                                 OR S.DiscountRecurring <> T.DiscountRecurring
                                 OR S.DiscountPriority <> T.DiscountPriority
                                 OR S.DiscountNet <> T.DiscountNet
                 ...
                 THEN UPDATE SET
                                 T.DiscountTypeName = S.DiscountTypeName,
                                 T.DiscountTypeDescription = S.DiscountTypeDescription,
                                 T.DiscountCategory = S.DiscountCategory,
                                 T.DiscountRecurring = S.DiscountRecurring,
                                 T.DiscountPriority = S.DiscountPriority,
                                 T.DiscountNet = S.DiscountNet,
		  ...
		  -- Debug output progress
		   ...
		   -- Drop the temp table
		   --
             DROP TABLE #DimDiscountTypeUpsert;
