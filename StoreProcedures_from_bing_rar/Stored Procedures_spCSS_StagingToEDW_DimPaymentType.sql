CREATE PROCEDURE dbo.spCSS_StagingToEDW_DimPaymentType
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCSS_StagingToEDW_DimPaymentType
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --                         the DimPaymentType table from Staging to BING_EDW.
    --
    ...
    --
    -- Usage:              EXEC dbo.spCSS_StagingToEDW_DimPaymentType @DebugMode = 1	
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimPaymentType - CSS';
	    ...
	    --
         ...
         --
         -- ================================================================================
         CREATE TABLE #DimPaymentTypeUpsert
         ...
         -- Create helper index
         --
         CREATE NONCLUSTERED INDEX XAK1DimPaymentTypeUpsert ON #DimPaymentTypeUpsert
         ([SourceSystem] ASC, [CSSTransactionCode] ASC
         );
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
             MERGE [BING_EDW].[dbo].[DimPaymentType] T
             USING #DimPaymentTypeUpsert S
             ON(S.SourceSystem = T.SourceSystem
                AND S.CSSTransactionCode = T.CSSTransactionCode)
                 WHEN MATCHED AND(S.PaymentTypeID <> T.PaymentTypeID
                                  OR S.PaymentTypeName <> T.PaymentTypeName)
                 THEN UPDATE SET
                                 T.PaymentTypeID = S.PaymentTypeID,
                                 T.PaymentTypeName = S.PaymentTypeName,
