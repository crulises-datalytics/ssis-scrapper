...
(
    ...
    FROM sys.objects
    WHERE type = 'P'
          AND name = 'spCMS_StagingToEDW_DimAdjustmentReason'
)
    DROP PROCEDURE dbo.spCMS_StagingToEDW_DimAdjustmentReason;
GO
*/

CREATE PROCEDURE dbo.spCMS_StagingToEDW_DimAdjustmentReason
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_StagingToEDW_DimAdjustmentReason
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --                         the DimAdjustmentReason table from Staging to BING_EDW.
    --
    ...
    --
    -- Usage:              EXEC dbo.spCMS_StagingToEDW_DimAdjustmentReason @DebugMode = 1	
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimAdjustmentReason';
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
		   -- ================================================================================
             CREATE TABLE #DimAdjustmentReasonUpsert
             ([AdjustmentReasonID]   [INT] NOT NULL,
              [AdjustmentReasonName] [VARCHAR](200) NOT NULL,
		   ...
		   --
		   -- ================================================================================
             INSERT INTO #DimAdjustmentReasonUpsert
             EXEC dbo.spCMS_StagingTransform_DimAdjustmentReason
                  @EDWRunDateTime;
		   ...
		   -- Get how many rows were extracted from source 
             ...
             FROM #DimAdjustmentReasonUpsert;
		   ...
		   -- Create helper index
		   --
             CREATE NONCLUSTERED INDEX XAK1DimAdjustmentReasonUpsert ON #DimAdjustmentReasonUpsert
		   ...
		   --
             MERGE [BING_EDW].[dbo].[DimAdjustmentReason] T
             USING #DimAdjustmentReasonUpsert S
             ON(S.AdjustmentReasonID = T.AdjustmentReasonID)
                 WHEN MATCHED AND S.AdjustmentReasonName <> T.AdjustmentReasonName
                                  AND S.Deleted IS NULL
                 THEN UPDATE SET
                                 T.AdjustmentReasonName = S.AdjustmentReasonName,
		  ...
		  -- Debug output progress
		   ...
		   -- Drop the temp table
		   --
             DROP TABLE #DimAdjustmentReasonUpsert;
