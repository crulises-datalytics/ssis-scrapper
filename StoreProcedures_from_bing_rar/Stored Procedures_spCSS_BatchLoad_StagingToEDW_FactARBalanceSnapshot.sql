CREATE PROCEDURE [dbo].[spCSS_BatchLoad_StagingToEDW_FactARBalanceSnapshot]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCSS_BatchLoad_StagingToEDW_FactARBalanceSnapshot
    --
    ...
    --                         core logic.
    --                     @DaysBack - For FactNetRevenue load we have to go back and potentially reporcess
    ...
    --
    -- Usage:              EXEC dbo.spCSS_BatchLoad_StagingToEDW_FactARBalanceSnapshot @EDWRunDateTime = @EDWRunDateTime, @DebugMode = 1
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @TaskName VARCHAR(100)= 'FactARBalanceSnapshot - CSS';
	    ...
	    --                         -----  
	    --
	    -- There is also a CMS version of the proc spCSS_BatchLoad_StagingToEDW_FactARBalanceSnapshot
		...
		BEGIN TRY
		   ...
		   -- ================================================================================
             ...
             INSERT INTO #OriginalProcessingRange ( [BatchIdentifier] ) --@BatchByFiscalWeekNumber(FiscalWeekNumber)
                    ...
                    WHERE AsOfFiscalWeek >= @StartingFiscalWeekNumber;
		   ...
		   --
             ...
             FETCH NEXT FROM csr_fact_rabalance_fisc_weekend INTO @FiscalWeekNumber;
             WHILE @@FETCH_STATUS = 0
                 BEGIN
                     SELECT @DebugMsg = @ProcName+' : '+CONVERT(NVARCHAR(20), GETDATE())+' - Processing data for Fiscal Wek Number '+CONVERT(VARCHAR(10), @FiscalWeekNumber);
                     PRINT @DebugMsg;
				 -- ================================================================================
				 -- Execute the main StagingToEDW stored proc to load data for the given batch
				 -- ================================================================================
                     EXEC dbo.spCSS_StagingToEDW_FactARBalanceSnapshot
                          @AuditId = @AuditId,
