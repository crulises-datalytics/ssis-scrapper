CREATE PROCEDURE [dbo].[spMISC_BatchLoad_StagingToEDW_FactLaborSalary]
	@EDWRunDateTime DATETIME2 = NULL,
	@DebugMode      INT       = NULL
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spMISC_BatchLoad_StagingToEDW_FactLaborSalary
    --
    ...
    --
    --
    --Usage:              EXEC dbo.spMISC_BatchLoad_StagingToEDW_FactLaborSalary
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
    ...
    -- 02/07/2018		Banandesi			BNG-4764 Added OrgKey 
		...
		*/
	    ...
	    --
         ...
         BEGIN TRY
		   ...
		   -- ================================================================================
             -- DECLARE @BatchByPayRollDate TABLE(PayRollDate DATE);
             ...
             INSERT INTO @SourceFiscalDate(FiscalDate)
                    SELECT DISTINCT CAST(PayrollDate AS DATE)
                 FROM dbo.vLaborHoursCosting(NOLOCK)
				 WHERE StgModifiedDate > @LastProcessedDate;
             
			 INSERT INTO #OriginalProcessingRange ( [BatchIdentifier] ) --@BatchByPayRollDate(PayRollDate)
                    SELECT DISTINCT
                           FiscalWeekEndDate
			 ...
			 CREATE TABLE #FinalProcessingRange ( [PayRollDate] DATE );
		   ...
		   -- ================================================================================
             ...
             FOR
                 SELECT PayRollDate
                 FROM #FinalProcessingRange --@BatchByPayRollDate
		   ...
		   --
             ...
             FETCH NEXT FROM csr_fact_laborhour_fisc_weekend INTO @PayRollDate;
             WHILE @@FETCH_STATUS = 0
                 BEGIN
                     SELECT @DebugMsg = @ProcName+' : '+CONVERT(NVARCHAR(20), GETDATE())+' - Processing data for Fiscal Week End Date '+CONVERT(VARCHAR(10), @PayRollDate);
                     PRINT @DebugMsg;
				 -- ================================================================================
				 -- Execute the main StagingToEDW stored proc to load data for the given batch
				 -- ================================================================================
                     EXEC dbo.spMISC_StagingToEDW_FactLaborSalary
                          @AuditId = @AuditId,
