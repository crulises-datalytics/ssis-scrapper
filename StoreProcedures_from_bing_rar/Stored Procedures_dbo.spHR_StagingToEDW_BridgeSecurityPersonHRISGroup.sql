...
(
    ...
    FROM sys.objects
    WHERE type = 'P'
          AND name = 'spHR_StagingToEDW_BridgeSecurityPersonHRISGroup'
)
    DROP PROCEDURE dbo.spHR_StagingToEDW_BridgeSecurityPersonHRISGroup;
GO
*/
CREATE PROCEDURE [dbo].[spHR_StagingToEDW_BridgeSecurityPersonHRISGroup]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spHR_StagingToEDW_BridgeSecurityPersonHRISGroup
    --
    -- Purpose:            Performs the Insert / Update ETL process for
    --                         the BridgeSecurityPersonHRISGroup table from Staging to BING_EDW.
    --
    --                         Step 1: Create temporary landing #table
    --                         Step 2: Populate the Landing table from Source by calling
    --                                 sub-procedure spHR_StagingTransform_BridgeSecurityPersonHRISGroup, 
    ...
    --
    -- Usage:              EXEC dbo.spHR_StagingToEDW_BridgeSecurityPersonHRISGroup @DebugMode = 1	
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'BridgeSecurityPersonHRISGroup';
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
		   -- ================================================================================
             CREATE TABLE #BridgeSecurityPersonHRISGroupUpsert
             (
	          [PersonKey] [int] NOT NULL,
	          [HRISGroupNumber] [int] NOT NULL,
		   ...
		   --
		   -- ================================================================================
             INSERT INTO #BridgeSecurityPersonHRISGroupUpsert
             EXEC dbo.spHR_StagingTransform_BridgeSecurityPersonHRISGroup
                  @EDWRunDateTime;

		   -- Get how many rows were extracted FROM source 

             SELECT @SourceCount = COUNT(1)
             FROM #BridgeSecurityPersonHRISGroupUpsert;
		   ...
		   -- Create helper index
		   --
             CREATE NONCLUSTERED INDEX XAK1BridgeSecurityPersonHRISGroupUpsert ON #BridgeSecurityPersonHRISGroupUpsert
             ([PersonKey] ASC
             );
		   ...
		   --
             MERGE [BING_EDW].[dbo].[BridgeSecurityPersonHRISGroup] T
             USING #BridgeSecurityPersonHRISGroupUpsert S
             ON(S.PersonKey = T.PersonKey)
                 WHEN MATCHED AND (S.HRISGroupNumber  <>  T.HRISGroupNumber
               )
                 THEN UPDATE SET
                 T.HRISGroupNumber = S.HRISGroupNumber,
		  ...
		  -- Debug output progress
		   ...
		   -- Drop the temp table
		   --
             DROP TABLE #BridgeSecurityPersonHRISGroupUpsert;
