...
(
    ...
    FROM sys.objects
    WHERE type = 'P'
          AND name = 'spMISC_StagingToEDW_FactLaborSalary'
)
    DROP PROCEDURE dbo.spMISC_StagingToEDW_FactLaborSalary;
GO
*/
CREATE  PROCEDURE [dbo].[spMISC_StagingToEDW_FactLaborSalary]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spMISC_StagingToEDW_FactLaborSalary
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --                         the FactLaborSalary table from Staging to BING_EDW.
    --
    --                     Step 1: Create temporary landing #table
    --                     Step 2: Populate the Landing table from Source by calling
    --                         sub-procedure spMISC_StagingTransfrom_FactLaborSalary, 
    ...
    --                     @PayRollDate - Pay Roll Date.  This proc has the option
    --                         of only returning data for a given Pay roll date, which can 
    ...
    --
    --
    -- Usage:              EXEC dbo.spMISC_StagingToEDW_FactLaborSalary @PayRoll = '20170204'
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
	...
	--04/22/2019   hhebbalu          BI-851 SourceSystem is not defined
    --			 
    -- ================================================================================
     BEGIN
         SET NOCOUNT ON;
	    ...
	    --
         ...
         DECLARE @TaskName VARCHAR(100)= 'FactLaborSalary';

	    --
	    -- For larger Fact (and Dimension) loads we will split the loads
	    --
         DECLARE @BatchSplitByName VARCHAR(50)= 'PayRollDate';
         DECLARE @BatchSplitByValue INT= CONVERT(INT, CAST(DATEPART(YYYY, @PayRollDate) AS [CHAR](4))+RIGHT('0'+CAST(DATEPART(M, @PayRollDate) AS [VARCHAR](2)), 2)+RIGHT('0'+CAST(DATEPART(D, @PayRollDate) AS [VARCHAR](2)), 2));
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
		   -- ================================================================================
             CREATE TABLE #FactLaborSalaryUpsert
             (
	...
	[EDWCreatedDate] [datetime] NOT NULL
		   ...
		   --
		   -- ================================================================================
             INSERT INTO #FactLaborSalaryUpsert
             EXEC dbo.spMISC_StagingTransform_FactLaborSalary
                  @EDWRunDateTime = @EDWRunDateTime,
                  @PayRollDate = @PayRollDate; -- @PayRollDate needs to be provided


		   -- Get how many rows were extracted from source 

             SELECT @SourceCount = COUNT(1)
             FROM #FactLaborSalaryUpsert;
		   ...
		   --
             DELETE BING_EDW.dbo.FactLaborSalary
             FROM BING_EDW.dbo.FactLaborSalary T
                  INNER JOIN #FactLaborSalaryUpsert S ON S.DateKey = T.DateKey ;
		   ...
		   --
             INSERT INTO BING_EDW.dbo.FactLaborSalary
             (
            [DateKey]
           ,[EmployeeNumber]       
		  ...
		  -- Debug output progress
		   ...
		   -- Drop the temp table
		   --
             DROP TABLE #FactLaborSalaryUpsert;
