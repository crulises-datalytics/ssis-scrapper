...
(
    ...
    FROM sys.objects
    WHERE type = 'P'
          AND name = 'spCSS_StagingToEDW_FactFTESnapshot'
)
    DROP PROCEDURE dbo.spCSS_StagingToEDW_FactFTESnapshot;
GO
*/
CREATE PROCEDURE [dbo].[spCSS_StagingToEDW_FactFTESnapshot]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCSS_StagingToEDW_FactFTESnapshot
    --
    ...
    --
    -- Usage:              EXEC dbo.spCSS_StagingToEDW_FactFTESnapshot @DebugMode = 1  
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:         
    ...
    -- ----        -----------         --------
    --
    -- 02/20/18     anmorales          BNG-249 - EDW FactFTESnapshot - CSS: Created procedure
    --           
    -- ================================================================================
         BEGIN
             SET NOCOUNT ON;
        ...
        -- --------------------------------------------------------------------------------
           ...
           -- ================================================================================
                 ...
                 EXEC dbo.spCSS_StagingTransform_FactFTESnapshot
           ...
           --
           CREATE NONCLUSTERED INDEX XAK1FactFTESnapshotUpsert ON #FactFTESnapshotUpsert
           ([DateKey] ASC, 
		  [SourceSystem] ASC
           );
           CREATE UNIQUE NONCLUSTERED INDEX XAK2FactFTESnapshotUpsert ON #FactFTESnapshotUpsert
           ([DateKey] ASC,
            [ReferenceID] ASC
           );
           ...
           --
             ...
             FROM BING_EDW.dbo.FactFTESnapshot T
                  INNER JOIN (SELECT DateKey, SourceSystem FROM #FactFTESnapshotUpsert GROUP BY DateKey, SourceSystem) S ON S.DateKey = T.DateKey
