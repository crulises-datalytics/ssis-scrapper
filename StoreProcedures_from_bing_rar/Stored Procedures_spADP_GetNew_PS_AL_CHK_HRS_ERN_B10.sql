CREATE PROCEDURE [dbo].[spADP_GetNew_PS_AL_CHK_HRS_ERN_B10]
...
AS
	-- ================================================================================
	-- 
	-- Stored Procedure:   spADP_GetNew_PS_AL_CHK_HRS_ERN_B10
	--
	-- Purpose:            Fetches records from [dbo].[ADP_PS_AL_CHK_HRS_ERN_B0] for insert into
	--					   [dbo].[ADP_PS_AL_CHK_HRS_ERN_B10]
	--
	...
	BEGIN
		...
		BEGIN TRY;
			...
			TRUNCATE TABLE [dbo].[ADP_PS_AL_CHK_HRS_ERN_B0_TMP];
			...
			--
			IF NOT EXISTS (SELECT 1 FROM [sys].[tables] WHERE [name] = 'ADP_PS_AL_CHK_HRS_ERN_B10_TMP' AND [schema_id] = SCHEMA_ID('dbo'))
				BEGIN
					CREATE TABLE [dbo].[ADP_PS_AL_CHK_HRS_ERN_B10_TMP] (
						[paygroup] [varchar](3) NOT NULL,
						[file_nbr] [varchar](16) NOT NULL,
						[check_dt] [date] NOT NULL,
						[week_nbr] [tinyint] NOT NULL,
			...
			TRUNCATE TABLE [dbo].[ADP_PS_AL_CHK_HRS_ERN_B10_TMP];
			INSERT INTO [dbo].[ADP_PS_AL_CHK_HRS_ERN_B10_TMP] (
				[paygroup],
				[file_nbr],
				[check_dt],
				[week_nbr],
			...
			SELECT
				...
				[datalakeinserttime],
				[RowHash]
			FROM [dbo].[ADP_PS_AL_CHK_HRS_ERN_B10]
			WHERE YEAR([check_dt]) = @ProcessingYear;
			...
			SELECT
				...
				[datalakeinserttime],
				[RowHash]
			FROM [dbo].[ADP_PS_AL_CHK_HRS_ERN_B0_TMP] AS [B0]
			WHERE NOT EXISTS (
				SELECT 1
				FROM [dbo].[ADP_PS_AL_CHK_HRS_ERN_B10_TMP] AS [B10]
