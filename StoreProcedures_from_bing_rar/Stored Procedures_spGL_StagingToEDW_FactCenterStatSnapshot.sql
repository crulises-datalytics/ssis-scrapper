CREATE PROCEDURE dbo.spGL_StagingToEDW_FactCenterStatSnapshot
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spGL_StagingToEDW_FactCenterStatSnapshot
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --					  the FactCenterStatSnapshot table from Staging to BING_EDW.
    --
    --                     Step 1: Create temporary landing #table
    --                     Step 2: Populate the Landing table from Source by calling
    --                         sub-procedure spGL_StagingTransform_FactCenterStatSnapshot, 
    ...
    --
    --
    -- Usage:              EXEC dbo.spGL_StagingToEDW_FactAdjustment @EDWRunDateTime = @EDWRunDateTime, @DebugMode = 1
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
     ...
     BEGIN
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
		   -- ================================================================================
             CREATE TABLE #FactCenterStatSnapshotUpsert
             (FiscalWeekEndDateKey INT NOT NULL,
              OrgKey               INT NOT NULL,
              ...
              LicensedCapacity     INT NOT NULL,
              BuildingCapacity     INT NOT NULL,
              OperationalCapacity  INT NOT NULL,
		   ...
		   --
		   -- ================================================================================
             INSERT INTO #FactCenterStatSnapshotUpsert
             EXEC dbo.spGL_StagingTransform_FactCenterStatSnapshot
		   ...
		   -- Get how many rows were extracted from source 
             ...
             FROM #FactCenterStatSnapshotUpsert;
		   ...
		   --
             DELETE BING_EDW.dbo.FactCenterStatSnapshot
             FROM BING_EDW.dbo.FactCenterStatSnapshot T
                  INNER JOIN #FactCenterStatSnapshotUpsert S ON S.FiscalWeekEndDateKey = T.FiscalWeekEndDateKey;
		   ...
		   --
             INSERT INTO BING_EDW.dbo.FactCenterStatSnapshot
                    SELECT *
                    FROM #FactCenterStatSnapshotUpsert;
		  ...
		  -- Debug output progress
		   ...
		   -- Drop the temp table
		   --
             DROP TABLE #FactCenterStatSnapshotUpsert;
