CREATE PROCEDURE dbo.spCSS_StagingToEDW_DimTransactionCode
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCSS_StagingToEDW_DimTransactionCode
    --
    -- Purpose:            Performs the Insert / Update / Delete ETL process for
    --                         the DimTransactionCode table from Staging to BING_EDW.
    --
    ...
    --
    -- Usage:              EXEC dbo.spCSS_StagingToEDW_DimTransactionCode @DebugMode = 1	
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimTransactionCode - CSS';
	    ...
	    --
         ...
         --
         -- ================================================================================
         CREATE TABLE #DimTransactionCodeUpsert
         ([TransactionCode]     VARCHAR(8) NOT NULL,
          [TransactionCodeName] VARCHAR(100) NOT NULL,
          [TransactionTypeCode] VARCHAR(2) NOT NULL,
          [TransactionTypeName] VARCHAR(100) NOT NULL,
          [TransactionCodeFTE]  NUMERIC(5, 3) NOT NULL,
         ...
         --
         -- ================================================================================
         INSERT INTO #DimTransactionCodeUpsert
         EXEC dbo.spCSS_StagingTransform_DimTransactionCode;
		   
         -- Get how many rows were extracted from source 

         SELECT @SourceCount = COUNT(1)
         FROM #DimTransactionCodeUpsert;
         ...
         -- Create helper index
         --
         CREATE NONCLUSTERED INDEX XAK1DimTransactionCodeUpsert ON #DimTransactionCodeUpsert
         ([TransactionCode] ASC, [TransactionTypeCode] ASC
         );
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
             MERGE [BING_EDW].[dbo].[DimTransactionCode] T
             USING #DimTransactionCodeUpsert S
             ON(S.TransactionCode = T.TransactionCode
                AND S.TransactionTypeCode = T.TransactionTypeCode)
                 WHEN MATCHED AND(S.TransactionCodeName <> T.TransactionCodeName
                                  OR S.TransactionTypeName <> T.TransactionTypeName
                                  OR S.TransactionCodeFTE <> T.TransactionCodeFTE
                                  OR S.EDWEffectiveDate <> T.EDWEffectiveDate
                                  OR S.EDWEndDate <> T.EDWEndDate)
                 THEN UPDATE SET
                                 T.TransactionCodeName = S.TransactionCodeName,
                                 T.TransactionTypeName = S.TransactionTypeName,
                                 T.TransactionCodeFTE = S.TransactionCodeFTE,
                                 T.EDWEffectiveDate = S.EDWEffectiveDate,
                                 T.EDWEndDate = S.EDWEndDate,
