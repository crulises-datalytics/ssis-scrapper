Create PROCEDURE [dbo].[spCMS_StagingToEDW_DimScheduleType]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_StagingToEDW_DimScheduleType
    --
    -- Purpose:            Performs the Insert / Update ETL process for
    --                         the DimScheduleType table from Staging to BING_EDW.
    --
    --                         Step 1: Create temporary landing #table
    --                         Step 2: Populate the Landing table from Source by calling
    --                                 sub-procedure spCMS_StagingTransform_DimScheduleType, 
    --                                 and create any helper indexes
    --                         Step 3: Perform the Insert / Update  required for this EDW
    --                                 table load
    ...
    --
    -- Usage:              EXEC dbo.spCMS_StagingToEDW_DimPaymentType @DebugMode = 1
    --
    -- --------------------------------------------------------------------------------
    --
    -- Change Log:		   
    ...
    -- ----        -----------         --------
    --
    -- 2/21/18     Adevabhakthuni             BNG-1240 - Refactor DimScheduleType ETL so it uses 
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimScheduleType';
	    ...
	    --
	    -- Merge statement action table variable
	    --
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   --
		   -- ================================================================================
             CREATE TABLE #DimScheduleType
			 (
		[ScheduleTypeID] [int] NOT NULL,
	[ScheduleTypeName] [varchar](50) NOT NULL,
	[ScheduleTypeMinimumDays] [int] NOT NULL,
	[ScheduleTypeMaximumDays] [int] NOT NULL,
	[ScheduleGroup] [varchar](50) NOT NULL,
	...
	[Deleted] [datetime2](7) NULL
		   ...
		   -- Create helper index
		   --
             CREATE NONCLUSTERED INDEX XAK1DimFeeType ON #DimScheduleType
             ([ScheduleTypeID] ASC
             );
		   ...
		   -- S T E P   3.
		   --
		   -- Perform the Inserts for new records, and SCD Type 1 for updated records.
		   --
		   -- The first MERGE statement performs the inserts for any new rows, and the first
		   -- part of the SCD1 update process for changed existing records, but setting the
		   ...
		   --
             MERGE [BING_EDW].[dbo].[DimScheduleType] T
             USING #DimScheduleType S
             ON(S.ScheduleTypeID = T.ScheduleTypeID)
                 WHEN MATCHED AND S.Deleted IS NULL
                                      AND ( 
									 T.ScheduleTypeName <> S.ScheduleTypeName
								  OR T.ScheduleTypeMinimumDays <> S.ScheduleTypeMinimumDays
								  OR T.ScheduleTypeMaximumDays <> S.ScheduleTypeMaximumDays
								  OR T.ScheduleGroup <> S.ScheduleGroup
   								  OR T.Deleted IS NOT NULL
                                      )
                 THEN UPDATE SET
								 T.ScheduleTypeName = S.ScheduleTypeName,
								 T.ScheduleTypeMinimumDays = S.ScheduleTypeMinimumDays,
								 T.ScheduleTypeMaximumDays = S.ScheduleTypeMaximumDays,
...
S.ScheduleTypeID,
S.ScheduleTypeName,
S.ScheduleTypeMinimumDays,
S.ScheduleTypeMaximumDays,
S.ScheduleGroup,
S.EDWCreatedDate,
S.EDWCreatedBy,
S.EDWModifiedDate,
