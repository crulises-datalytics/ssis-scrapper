CREATE PROCEDURE dbo.spCSS_StagingToEDW_DimStudent
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCSS_StagingToEDW_DimStudent
    --
    ...
    --
    -- Usage:              EXEC dbo.spCSS_StagingToEDW_DimStudent @DebugMode = 1	
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimStudent - CSS';
	    ...
	    --
         ...
         ([SourceSystem] ASC, [CSSCenterNumber] ASC, [CSSFamilyNumber] ASC, [CSSStudentNumber] ASC
         );
         ...
         BEGIN TRY
		   ...
		   -- Debug output progress
             ...
             ON(S.SourceSystem = T.SourceSystem
                AND S.CSSCenterNumber = T.CSSCenterNumber
                AND S.CSSFamilyNumber = T.CSSFamilyNumber
                AND S.CSSStudentNumber = T.CSSStudentNumber)
                 WHEN MATCHED AND T.EDWEndDate IS NULL -- The 'current' record in target
             ...
             (EDWEndDate,
              ...
              EDWCreatedBy
                    ...
                    FROM #DimStudentUpsert S
                         INNER JOIN @tblMrgeActions_SCD2 scd2 ON S.CSSCenterNumber = scd2.CSSCenterNumber
                                                                 AND S.CSSFamilyNumber = scd2.CSSFamilyNumber
                                                                 AND S.CSSStudentNumber = scd2.CSSStudentNumber
