...
(
    ...
    FROM sys.objects
    WHERE type = 'P'
          AND name = 'spCMS_StagingToEDW_DimProgram'
)
    DROP PROCEDURE dbo.spCMS_StagingToEDW_DimProgram;
GO
*/
CREATE PROCEDURE [spCMS_StagingToEDW_DimProgram]
...
AS
    -- ================================================================================
    -- 
    -- Stored Procedure:   spCMS_StagingToEDW_DimProgram
    --
    ...
    --
    -- Usage:              EXEC dbo.spCMS_StagingToEDW_DimProgram @DebugMode = 1	
    -- 
    --
    -- --------------------------------------------------------------------------------
    --
    ...
    -- ----        -----------         --------
    --
    -- 02/21/18    hhebbalu           BNG-1239 - Refactor DimProgram ETL so it uses Stored Proc over DFTs 
	--								  creating temporary DB Objects
    ...
    -- ================================================================================
     BEGIN
	    ...
	    --
         ...
         DECLARE @SourceName VARCHAR(100)= 'DimProgram';
	    ...
	    -- --------------------------------------------------------------------------------
         BEGIN TRY
		   ...
		   -- Create helper index
		   --
             CREATE UNIQUE NONCLUSTERED INDEX XAK1DimDiscountTypeUpsert ON #DimProgramUpsert
             ([SourceSystem] ASC, [ProgramID] ASC
             );
		   ...
		   --
             ...
             ON(S.SourceSystem = T.SourceSystem AND S.ProgramID = T.ProgramID)
                 WHEN MATCHED AND(S.ProgramName <> T.ProgramName
